...
(2025/09/30) Codex CLI のバージョンも作りましたのでこちらもぜひ🤲
...
## \--dangerously-skip-permissions を使う場合

`claude --dangerously-skip-permissions` で実行すると、通常は都度表示される「このコマンドを実行してもよいか？」といった権限確認プロンプト（許可ダイアログ）をすべてスキップし、AIエージェントが自律的にファイル操作やコマンド実行を進めます。

もし `--dangerously-skip-permissions` を使いたい場合にはこちらの対応を行なっておくと安心かもです。

(`$HOME/.claude/settings.json` の `permissions.deny` も合わせて設定が必要です)
...
Claude Codeを賢くしてくれると評判の [Serena](https://github.com/oraios/serena) の紹介です。 [mise](https://github.com/jdx/mise) を使っているのでディレクトリで以下を実行。

```bash
mise use uv
```

こちらのコマンドを実行するだけでMCPとして認識してくれます。
...
[こちらの記事](https://zenn.dev/sc30gsw/articles/ff81891959aaef#1.-custom-slash-command) の中に Custom Command や Sub Agent で Serena を活用するサンプルがあるのでよかったら試してみてください。
